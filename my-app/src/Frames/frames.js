import people1 from './assets/images/people1.jpg';
import people2 from './assets/images/people2.jpg';
import people3 from './assets/images/people3.jpg';
import people4 from './assets/images/people4.jpg';
import people5 from './assets/images/people5.jpg';
import people6 from './assets/images/people6.jpg';
import art1 from './assets/images/art1.jpg';
import art2 from './assets/images/art2.jpg';
import art3 from './assets/images/art3.jpg';
import art4 from './assets/images/art4.jpg';
import art5 from './assets/images/art5.jpg';
import art6 from './assets/images/art6.jpg';
import fashion1 from './assets/images/fashion1.jpeg';
import fashion2 from './assets/images/fashion2.jpg';
import fashion3 from './assets/images/fashion3.jpg';
import fashion4 from './assets/images/fashion4.jpg';
import fashion5 from './assets/images/fashion5.jpg';
import fashion6 from './assets/images/fashion6.jpeg';
import fashion7 from './assets/images/fashion7.jpg';
import fashion8 from './assets/images/fashion8.jpg';
import fashion9 from './assets/images/fashion9.jpg';
import fashion10 from './assets/images/fashion10.jpg';

export const FRAMES = [
    {
        id: 1,
        title: 'PEOPLE',
        text: 'people need people.',
        imgs: [
            {src: people1, className: 'people-1'},
            {src: people2, className: 'people-2'},
            {src: people3, className: 'people-3'},
            {src: people4, className: 'people-4'},
            {src: people5, className: 'people-5'},
            {src: people6, className: 'people-6'},
        ],
    },
    {
        id: 2,
        title: 'ART',
        text: 'art saves souls.',
        imgs: [
            {src: art1, className: 'art-1'},
            {src: art2, className: 'art-2'},
            {src: art3, className: 'art-3'},
            {src: art4, className: 'art-4'},
            {src: art5, className: 'art-5'},
            {src: art6, className: 'art-6'},
        ],
    },
    {
        id: 3,
        title: 'FASHION',
        text: 'fashion heals.',
        imgs: [
            {src: fashion1, className: 'fashion-1'},
            {src: fashion2, className: 'fashion-2'},
            {src: fashion3, className: 'fashion-3'},
            {src: fashion4, className: 'fashion-4'},
            {src: fashion5, className: 'fashion-5'},
            {src: fashion6, className: 'fashion-6'},
            {src: fashion7, className: 'fashion-7'},
            {src: fashion8, className: 'fashion-8'},
            {src: fashion9, className: 'fashion-9'},
            {src: fashion10, className: 'fashion-10'},
        ],
    },
];