...
function CreatorDashboard() {
  ...
  const [showNewFrame, setShowNewFrame] = useState<boolean>(false)
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`${API}/api/creators/me`, { headers: { Authorization: `Bearer ${token}` } })
        .then(r => r.ok ? r.json() : null),
      fetch(`${API}/api/creators/me/frames`, { headers: { Authorization: `Bearer ${token}` } })
        .then(r => r.ok ? r.json() : [])
    ]).then(([profile, myFrames]: [Creator | null, Frame[]]) => {
  ...
  const handleLogout = (): void => {
    ;['authToken', 'userRole', 'username'].forEach(k => localStorage.removeItem(k))
    navigate('/')
  }

  const handleCreateFrame = async (): Promise<void> => {
    ...
    try {
      ...
      if (r.ok) {
        ...
        setFrames(prev => [{
          ...
          layout: '{}', is_published: true, created_at: new Date().toISOString(), tags: []
        }, ...prev])
        setNewTitle('')
        setNewDesc('')
  ...
  const handleDelete = async (frameId: number): Promise<void> => {
    ...
    <div className="creator-dashboard">
      <aside className="creator-sidebar">
        ...
        <div className="creator-profile">
          <div className="creator-avatar">{creator?.first_name?.charAt(0).toUpperCase()}</div>
          <div className="creator-name">{creator?.first_name} {creator?.last_name}</div>
          <div className="creator-username">@{creator?.username}</div>
          {creator?.portfolio_url && (
            <a href={creator.portfolio_url} target="_blank" rel="noopener noreferrer" className="creator-portfolio-link">
              портфолио ↗
            </a>
          )}
        </div>
        ...
        <div className="creator-stats">
          ...
          <div className="creator-stat">
            <span className="creator-stat-num">{frames.filter(f => f.is_published).length}</span>
      ...
      <main className="creator-main">
        <div className="creator-main-header">
          ...
          <button className="creator-add-btn" onClick={() => navigate('/editor')}>
            + новый мудборд
          </button>
        </div>
        ...
        {showNewFrame && (
          <div className="creator-new-frame">
            <input autoFocus className="creator-input" placeholder="название мудборда"
              value={newTitle} onChange={e => setNewTitle(e.target.value)}
              onKeyDown={e => e.key === 'Enter' && handleCreateFrame()} />
            <input className="creator-input" placeholder="описание (необязательно)"
              value={newDesc} onChange={e => setNewDesc(e.target.value)} />
            <div className="creator-new-frame-actions">
              <button className="creator-btn creator-btn--primary"
                onClick={handleCreateFrame} disabled={creating || !newTitle.trim()}>
                {creating ? 'создаю...' : 'создать'}
              </button>
              <button className="creator-btn creator-btn--ghost"
        ...
        {frames.length === 0 && !showNewFrame && (
          <div className="creator-empty">
            ...
            <button className="creator-add-btn" onClick={() => navigate('/editor')}>создать первый</button>
          </div>
        )}
          ...
          {frames.map(frame => (
            <div key={frame.id} className={`creator-frame-card ${!frame.is_published ? 'draft' : ''}`}>
              ...
              <div className="creator-frame-info">
                ...
                {frame.description && <p className="creator-frame-desc">{frame.description}</p>}
                {frame.tags?.length > 0 && (
                  <div className="creator-frame-tags">
                    {frame.tags.map(t => <span key={t.id} className="creator-frame-tag">{t.name}</span>)}
                  </div>
                )}
                <div className="creator-frame-actions">
                  <button className="creator-btn creator-btn--ghost" onClick={() => navigate(`/editor/${frame.id}`)}>
                    редактировать
                  </button>
                  {frame.is_published && (
                    <button className="creator-btn creator-btn--ghost" onClick={() => navigate(`/boards/${frame.id}`)}>
                      просмотр
                    </button>
                  )}
                  <button className="creator-btn creator-btn--danger" onClick={() => handleDelete(frame.id)}>
                    удалить
                  </button>
                </div>
              </div>
