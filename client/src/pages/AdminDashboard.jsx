...
function AdminDashboard() {
  ...
  const confirmApprove = () => {
    ...
    fetch(`http://localhost:8000/api/admin/creators/applications/${selectedApp.id}/approve`, {
      ...
      .then(res => res.json())
      .then(data => {
        alert(`Заявка одобрена!\n\nСсылка для пользователя:\n${data.activation_link}\n\nОтправьте её пользователю вручную (телеграм/инстаграм)`);
        ...
        <div className="admin-modal-overlay" onClick={() => setShowApproveModal(false)}>
          <div className="admin-modal-content" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-actions">
              <button onClick={confirmApprove} className="modal-btn approve-btn">
                Да, одобрить
              </button>
              <button onClick={() => setShowApproveModal(false)} className="modal-btn cancel-btn">
                Отмена
              </button>
            </div>
          </div>
      ...
      {showRejectModal && (
        <div className="admin-modal-overlay" onClick={() => setShowRejectModal(false)}>
          <div className="admin-modal-content" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-actions">
              <button onClick={confirmReject} className="modal-btn reject-btn">
                Да, отклонить
              </button>
              <button onClick={() => setShowRejectModal(false)} className="modal-btn cancel-btn">
                Отмена
              </button>
            </div>
          </div>
