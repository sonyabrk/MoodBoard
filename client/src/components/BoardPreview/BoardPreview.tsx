import type { BoardPreviewProps, LayoutItem, Layout } from '../../types'

const CANVAS_W = 1200
const CANVAS_H = 800

function BoardPreview({ layout }: BoardPreviewProps) {
  let items: LayoutItem[] = []
  try {
    const parsed: Layout = typeof layout === 'string' ? JSON.parse(layout) : layout
    items = parsed?.items || []
  } catch (err) {
  ...
  return (
    <div style={{ width: '100%', paddingBottom: `${aspectRatio * 100}%`, position: 'relative', overflow: 'hidden', background: '#fff' }}>
      ...
      >
        {items.map((item: LayoutItem, i: number) => (
          <div key={i} style={{
            position: 'absolute',
